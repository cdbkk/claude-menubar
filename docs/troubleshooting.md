...
- Confirm it's running with `pgrep -x ClaudeMenuBar`: a number means it's running (it may just be hidden) no output means it exited because no Claude session is active.
- If first-launch setup never took, run the installer manually: `node "/Applications/ClaudeMenuBar.app/Contents/Resources/install.js"`
