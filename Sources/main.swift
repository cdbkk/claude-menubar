...
final class StatusController: NSObject, NSMenuDelegate {
    ...
    let brand = NSColor(srgbRed: 0.106, green: 0.761, blue: 0.702, alpha: 1) // #1BC2B3 teal accent
    ...
    var showTimer = false
    var iconSystem = false // false = brand Teal; true = adaptive black/white (template image)
    ...
    func ensureHooksInstalled() {
        ...
        DispatchQueue.global().async {
            guard let node = Self.locateNode() else {
                NSLog("ClaudeMenuBar: could not find node; hooks not installed (will retry next launch)")
                return
            }
            let task = Process()
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        for (sys, name) in [(false, "Teal"), (true, "System")] {
        ...
        let q = NSMenuItem(title: "Quit Claude Menu Bar", action: #selector(quit), keyEquivalent: "q")
        q.target = self
        menu.addItem(q)
    }
    ...
    func evaluate() {
        ...
        default:          render(label: "", color: iconColor, animate: false, startedAt: 0) // done + idle: just the teal spark
        }
    }
    ...
    // Full color (isTemplate=false), so the Teal/System color setting does NOT apply here.
