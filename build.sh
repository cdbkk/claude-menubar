#!/bin/bash
# Builds ClaudeMenuBar.app (and optionally a .dmg with: ./build.sh --dmg).
set -euo pipefail
cd "$(dirname "$0")"

APP="build/ClaudeMenuBar.app"
BIN="$APP/Contents/MacOS/ClaudeMenuBar"
...
<dict>
  ...
  <key>CFBundleDisplayName</key><string>Claude Menu Bar</string>
  <key>CFBundleIdentifier</key><string>com.local.claudemenubar</string>
...
#        xcrun notarytool store-credentials "claude-menubar" \
...
# ad-hoc dev build (runnable locally; users need right-click > Open once).
# Set your own Apple Team ID to sign: TEAM_ID=XXXXXXXXXX ./build.sh --dmg
TEAM_ID="${TEAM_ID:-}"
NOTARY_PROFILE="${NOTARY_PROFILE:-claude-menubar}"
...
if [[ "${1:-}" == "--dmg" ]]; then
  ...
  hdiutil create -volname "Claude Menu Bar" -srcfolder "$STAGE" -ov -format UDRW build/rw.dmg >/dev/null
...
tell application "Finder"
  tell disk "Claude Menu Bar"
    ...
    set position of item "ClaudeMenuBar.app" of container window to {130, 150}
...
OSA
  ...
  cp "/Volumes/Claude Menu Bar/.DS_Store" "$STAGE/.DS_Store" 2>/dev/null || true
  hdiutil detach "$device" >/dev/null || true
  rm -f build/rw.dmg
  hdiutil create -volname "Claude Menu Bar" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
  rm -rf "$STAGE"
